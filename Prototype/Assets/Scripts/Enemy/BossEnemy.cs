...
{
    public GameObject m_SkillEffectPos;
    void Awake()
    {
        m_GetStat = m_Enemy;
    ...
    {
        Init();
        m_Enemy.m_HPBar = UIManager.Instance.BossHP;
...
//        Debug.Log("몬스터 공격 딜레이 종료");
    ...
    public void EffectSkill1Create()
    {
        GameObject newEffect = GameObject.Instantiate(EffectManager.Instance.BossSkillEffect);
        newEffect.transform.SetParent(m_SkillEffectPos.transform);
    ...
    public void EffectSkill2Create()
    {
        GameObject newEffect = GameObject.Instantiate(EffectManager.Instance.BossSkillEffect);
        newEffect.transform.SetParent(transform.FindChild("pos"));
        newEffect.transform.localPosition = Vector3.zero;
        newEffect.transform.localRotation = Quaternion.identity;
        Destroy(newEffect, 1.5f);
    }
    ...
    {
        StartCoroutine(UIManager.Instance.GameFinishPlay(UIManager.Instance.GameClear));
    }
}
