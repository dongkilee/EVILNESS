...
public class MiddleEnemy : AIController
{
    public int rand =-1;
    ...
    void Start()
    {
        m = Body.GetComponent<Renderer>().material;
        if (Body.name == "Object001")
            hide = m.GetFloat("_Hide");

        Init();
        if (m_WayPoint != null)
            rand = Random.Range(0, m_WayPoint.Length);
        else
        {
            m_WayPoint = new GameObject [1];
            m_WayPoint [0] = gameObject;
        }
        Debug.Log(rand);
...
//        if (!GameManager.Instance.NowGameSpawner().PrevStage.GetComponent<StageClose>().CloseCheck)
    ...
    {
        return m_StunCheck || m_GroggyStunCheck;
    }
    ...
    protected override void SetMove()
    {
        ...
        if (rand == -1) return;
        m_NowTargetPos = m_WayPoint [rand].transform.position;
