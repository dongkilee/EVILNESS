Shader "Custom/FX_Player_dash" {
	Properties {
		...
		[Space(30)]
		_dashFXMap ("_dashFXMap", 2D) = "white" {}
		_dashFXcolor2 ("_dashFXcolor", Color) = (1,1,1,1)
		_dashFXcolor ("_dashFXcolor", Color) = (1,1,1,1)
		_dashFXpow ("_dashFXpow", Range(1,50)) =0
	...
	SubShader {
		...
		sampler2D _dashFXMap;
		float _dashFXpow;
		fixed4 _dashFXcolor ;
		fixed4 _dashFXcolor2 ;

		void surf (Input IN, inout SurfaceOutput o) {

			fixed4 c = tex2D (_MainTex, IN.uv_MainTex);
			fixed4 dashFXMap1 = tex2D (_dashFXMap, IN.uv_dashFXMap);

			float3 n = UnpackNormal(tex2D (_BumpMap, IN.uv_MainTex));
			float3 m = tex2D (_MaskMap, IN.uv_MaskMap);
			o.Normal = n * _NormalRR;
			o.Albedo = dashFXMap1.rgb * 0.7 *_dashFXcolor2;
			o.Emission = dashFXMap1.rgb *0.15;
...
//		================================ dash 
			float3 dashFX ;
			float dashRim = saturate(dot(e.Normal, viewDir)) ;
			dashFX=1-dashRim;
			dashFX = pow(dashFX,_dashFXpow) *_dashFXcolor ;


		//Outpur Room
			float4 finalColor;
			finalColor.rgb = DiffCol + rimCol + SpecCol1 + dashFX;
